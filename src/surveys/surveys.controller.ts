...
export class SurveysController {
  ...
  @ApiOperation({
    summary:
      'Get my submissions with status and rejection reasons (For Mobile App)',
  })
  ...
  @ApiOperation({
    summary: 'Get recent pending surveys for notifications (Admin only)',
  })
  getRecentPending() {
  ...
  adminUpdate(
    @Param('id', ParseUUIDPipe) id: string,
    @Body() updateData: { reward_amount?: number; estimated_time?: number },
  ) {
    return this.surveysService.adminUpdate(
      id,
  ...
  @ApiOperation({
    summary:
      'Get matching users for a survey with potential filter overrides (Admin only)',
  })
  getMatchingUsers(
    @Param('id', ParseUUIDPipe) id: string,
    @Body() overrides?: any,
  ...
  @ApiOperation({
    summary: 'Match CSV data with survey submissions (Admin only)',
  })
  matchCSV(
    @Param('id', ParseUUIDPipe) id: string,
    @Body() body: { rows: { unique_id?: string; email?: string }[] },
  ) {
  ...
  updateSubmissionStatus(
    ...
    return this.surveysService.updateSubmissionStatus(
      id,
      body.status,
      body.reject_reason,
    );
  ...
  @ApiOperation({
    summary: 'Advanced CSV validation with multiple rules (Admin only)',
  })
  validateAdvanced(
    @Param('id', ParseUUIDPipe) id: string,
  ...
  @ApiOperation({
    summary: 'Validate CSV responses and update submission status (Admin only)',
  })
  validateCSV(
    @Param('id', ParseUUIDPipe) id: string,
    @Body()
    body: { rows: any[]; idCol: string; ansCol: string; correctVal: string },
  ) {
    return this.surveysService.validateCSVAnswers(
      id,
      body.rows,
      body.idCol,
      body.ansCol,
