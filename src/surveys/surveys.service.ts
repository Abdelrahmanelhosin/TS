...
export class SurveysService {
  ...
  async findAllForUser(userId: string) {
    ...
    const matchedSurveys = await this.prisma.surveys.findMany({
      where: {
        status: 'active',
        AND: [
          ...(conditions.length > 0 ? conditions : []),
          {
            // Exclude surveys user already has a submission for
            submissions: {
              none: { user_id: userId }
            }
          }
    ...
    // Filter by quota manually since Prisma doesn't support comparing count with a field in a single 'where' easily without raw SQL
    return matchedSurveys.filter(s => {
      const target = (s as any).target_audience || 0;
      const reached = s._count.submissions;
      return target === 0 || reached < target;
    });
  }
  ...
  private normalizeFilters(val: any) {
    ...
    const normalize = (v: any): string => {
      ...
      if (s === 'Önlisans' || lower === 'onlisans' || lower === 'önlisans')
        return 'onlisans';
      ...
      // Work Status
      if (s === 'Çalışıyor' || lower === 'çalışıyor' || lower === 'calisiyor')
        return 'calisiyor';
      if (
        s === 'Çalışmıyor' ||
        lower === 'çalışmıyor' ||
        ...
        return 'calismiyor';
      if (s === 'Öğrenci' || lower === 'öğrenci' || lower === 'ogrenci')
        return 'ogrenci';
      if (s === 'Emekli' || lower === 'emekli') return 'emekli';
      if (lower.includes('hanımı') || lower.includes('hanimi'))
        return 'ev_hanimi';
      ...
      if (lower.includes('girişimci') || lower.includes('girisimci'))
        return 'girisimci_isletme_sahibi';
      if (lower.includes('üst düzey') || lower.includes('ust duzey'))
        return 'ust_duzey_yonetici';
      if (lower.includes('orta düzey') || lower.includes('orta duzey'))
        return 'orta_duzey_yonetici';
      if (lower.includes('takım lideri') || lower.includes('takim lideri'))
        return 'alt_duzey_yonetici_takim_lideri';
      if (lower === 'çalışan' || lower === 'calisan') return 'calisan';

      // Sector (after Position to avoid collision with "Girişimci / İşletme Sahibi")
      if (lower.includes('özel sektör') || lower.includes('ozel sektor'))
        return 'ozel_sektor';
      if (lower.includes('kamu sektörü') || lower.includes('kamu sektoru'))
        return 'kamu_sektoru';
      if (
        (lower.includes('işletme sahibi') ||
          lower.includes('isletme sahibi')) &&
        (lower.includes('esnaf') ||
          lower.includes('zanaatk') ||
          lower.includes('kendi'))
      )
        return 'isletme_sahibi_esnaf_zanaatkar_kendi_isi';
      ...
      if (lower.includes('yazılımcı') || lower.includes('yazilimci'))
        return 'yazilimci_bilisim_uzmani';
      if (lower.includes('bankacılık') || lower.includes('bankacilik'))
        return 'bankacilik_finans_uzmani';
      if (
        lower.includes('insan kaynakları') ||
        lower.includes('insan kaynaklari')
      )
        return 'insan_kaynaklari_uzmani';
      if (lower.includes('satış') || lower.includes('satis'))
        return 'satis_pazarlama_halkla_iliskiler';
  ...
  async approve(id: string, updateData?: any) {
    try {
      ...
      // Basic fields
      if (updateData?.reward_amount !== undefined)
        data.reward_amount = updateData.reward_amount;
      if (updateData?.estimated_time !== undefined)
        data.estimated_time = updateData.estimated_time;
      ...
      if (updateData?.total_cost !== undefined)
        data.total_cost = updateData.total_cost;
      if (updateData?.target_audience !== undefined)
        data.target_audience = updateData.target_audience;

      const toArray = (val: any) => this.normalizeFilters(val);

      if (updateData?.target_gender)
        data.target_gender = toArray(updateData.target_gender);
      if (updateData?.target_age_group)
        data.target_age_group = toArray(updateData.target_age_group);
      if (updateData?.target_city)
        data.target_city = toArray(updateData.target_city);
      if (updateData?.target_education)
        data.target_education = toArray(updateData.target_education);
      if (updateData?.target_occupation)
        data.target_occupation = toArray(updateData.target_occupation);
      if (updateData?.target_sector)
        data.target_sector = toArray(updateData.target_sector);
      if (updateData?.target_position)
        data.target_position = toArray(updateData.target_position);
      if (updateData?.target_income)
        data.target_income = toArray(updateData.target_income);
      if (updateData?.target_nationality)
        data.target_nationality = toArray(updateData.target_nationality);

      // Handle both old and new frontend field names
      const workStatus =
        updateData?.target_employment_status || updateData?.target_work_status;
      if (workStatus) data.target_employment_status = toArray(workStatus);

      const maritalStatus =
        updateData?.target_marital_status || updateData?.target_marital;
      if (maritalStatus) data.target_marital_status = toArray(maritalStatus);

      const childCount =
        updateData?.target_child_count || updateData?.target_children;
      if (childCount) data.target_child_count = toArray(childCount);

      console.log(
        '[Approve] Updating survey with data:',
      ...
      this.notifyMatchingUsers(id, updateData?.selectedUserIds).catch((err) =>
  ...
  async getSubmissions(surveyId: string) {
    ...
    const subs = await this.prisma.$queryRawUnsafe<any[]>(
      `
            SELECT 
                ...
                p.gender, p.birth_date, p.city, p.education_level
            FROM public.submissions sub
            LEFT JOIN auth.users u ON sub.user_id = u.id
    ...
    return subs.map((s) => ({
      ...s,
      users: {
        email: s.email || 'Gölge Kullanıcı',
        profiles: {
          full_name: s.full_name || 'İsimsiz Katılımcı',
          phone: s.phone || '—',
  ...
  async updateSubmissionStatus(
    ...
    const updated = await this.prisma.submissions.update({
      ...
      data: {
        ...
        reject_reason:
          status === 'rejected'
            ? reject_reason || 'İptal nedeni belirtilmedi'
            : null,
      },
    });
  ...
  private async notifySubmissionDecision(
    userId: string,
    surveyId: string,
    status: string,
    reason?: string | null,
    ...
    const title =
      status === 'approved'
        ? 'تهانينا! تم قبول مشاركتك'
        : 'عذراً، تم رفض مشاركتك';
    const body =
      status === 'approved'
  ...
  async matchCSV(
    surveyId: string,
    csvRows: { unique_id?: string; email?: string }[],
  ) {
    ...
    for (const sub of submissions) {
      const trustScore = sub.users?.profiles?.trust_score ?? 100;
    ...
    for (const row of csvRows) {
      const match = submissions.find((s: any) => {
        ...
        if (row.email && s.users?.email)
          return s.users.email.toLowerCase() === row.email.toLowerCase();
        return false;
      });
      if (match) {
    ...
    const unmatchedSubmissions = submissions.filter(
  ...
  async getPaymentTable(surveyId: string) {
    ...
    return {
      ...
      rows: subs.map((s: any) => {
        ...
        const isShadow =
          metadata.shadow === true || metadata.new_shadow === true;
        const participantCode = metadata.unique_id || '';

        return {
          full_name: isShadow
            ? `Guest (Code: ${participantCode})`
            : s.full_name || s.email || '—',
  ...
  async validateAdvancedCSV(surveyId: string, rows: any[], rules: any[]) {
    console.log(
      `Starting Advanced Validation for survey ${surveyId} with ${rows.length} rows`,
    );
    ...
    for (const row of rows) {
      const uniqueId =
        row.unique_id || row['Unique ID'] || row['ID'] || row['Katılımcı Kodu'];
      if (!uniqueId) {
      ...
      let submission = submissions.find(
        (s) =>
          String(s.unique_id || s.metadata?.unique_id || '').trim() ===
      ...
      if (!submission) {
        try {
          ...
          console.log(
            `Importing ${isShadow ? 'shadow' : 'linked'} participant: ${uniqueId}`,
          );
      ...
      for (const rule of rules) {
        if (rule.type === 'equality') {
          const rowVal = String(row[rule.column] || '')
            .trim()
            .toLowerCase();
          const expectedVal = String(rule.value || '')
            .trim()
            .toLowerCase();
      ...
      const rejectReason =
        finalStatus === 'rejected' ? messages.join(', ') : null;
      ...
      this.notifySubmissionDecision(
        ...
        console.error(
          `Failed to notify user ${submission.user_id} during CSV validation:`,
          err.message,
        );
      });
  ...
  async validateCSVAnswers(
    ...
    for (const submission of submissions) {
      ...
      const userId = String(submission.user_id || '')
        .trim()
        .toLowerCase();
      const userEmail = String(submission.users?.email || '')
        .trim()
        .toLowerCase();

      if (!userId && !userEmail) {
        console.log(
          `[Validation] Skipping submission ${submission.id}: No user_id or Email.`,
        );
        results.skipped++;
        continue;
      }
      ...
      const csvRow = rows.find((r) => {
        ...
        return Object.values(r).some((val) => {
          ...
          return (
            (cleanUserId &&
              (cleanCell === cleanUserId || cellValue === userId)) ||
            (cleanEmail &&
              (cleanCell === cleanEmail || cellValue === userEmail))
          );
        });
      });
      ...
      if (!csvRow) {
        console.log(
          `[Validation] Match NOT found for userId: "${userId}", Email: "${userEmail}"`,
        );
        // Not found in CSV: User likely entered a wrong code in the survey tool
        await this.updateSubmissionStatus(
          submission.id,
          'rejected',
          "Kullanıcı id'yi yanlış girdiğiniz tespit edilmiştir.",
        );
        results.rejected++;
      } else {
        ...
        const ansKey = Object.keys(csvRow).find(
          (k) => k.trim().toLowerCase() === ansCol.trim().toLowerCase(),
        );

        if (!ansKey) {
          console.log(
            `[Validation] ERROR: Answer column "${ansCol}" NOT found in CSV row for userId "${userId}"`,
          );
          await this.updateSubmissionStatus(
            submission.id,
            'rejected',
            'Veri hatası: Cevap sütunu bulunamadı.',
          );
          results.rejected++;
          continue;
        }

        const userAnswer = String(csvRow[ansKey] || '')
          .trim()
          .toLowerCase();
        const expected = String(correctVal).trim().toLowerCase();

        console.log(
          `[Validation] Comparing for userId "${userId}": UserAnswer="${userAnswer}", Expected="${expected}"`,
        );
        ...
        if (userAnswer !== '' && userAnswer === expected) {
          ...
          console.log(
            `[Validation] REJECT: Trap Failed for userId "${userId}". Got "${userAnswer}", Expected "${expected}"`,
          );
          // Answered incorrectly: Trap question failure
          await this.updateSubmissionStatus(
            submission.id,
            'rejected',
            'Tuzak soruya yanlış cevap verildiği tespit edilmiştir.',
          );
          results.rejected++;
        }
      }
  ...
  async getMatchingUsers(surveyId: string, overrides?: any) {
    ...
    if (overrides) {
      survey = {
        ...survey,
        ...overrides,
        target_gender: overrides.target_gender
          ? this.normalizeFilters(overrides.target_gender)
          : survey.target_gender,
        target_age_group: overrides.target_age_group
          ? this.normalizeFilters(overrides.target_age_group)
          : survey.target_age_group,
        target_city: overrides.target_city
          ? this.normalizeFilters(overrides.target_city)
          : survey.target_city,
        target_education: overrides.target_education
          ? this.normalizeFilters(overrides.target_education)
          : survey.target_education,
        target_occupation: overrides.target_occupation
          ? this.normalizeFilters(overrides.target_occupation)
          : survey.target_occupation,
        target_employment_status: overrides.target_employment_status
          ? this.normalizeFilters(overrides.target_employment_status)
          : survey.target_employment_status,
        target_sector: overrides.target_sector
          ? this.normalizeFilters(overrides.target_sector)
          : survey.target_sector,
        target_position: overrides.target_position
          ? this.normalizeFilters(overrides.target_position)
          : survey.target_position,
        target_income: overrides.target_income
          ? this.normalizeFilters(overrides.target_income)
          : survey.target_income,
        target_marital_status: overrides.target_marital_status
          ? this.normalizeFilters(overrides.target_marital_status)
          : survey.target_marital_status,
        target_child_count: overrides.target_child_count
          ? this.normalizeFilters(overrides.target_child_count)
          : survey.target_child_count,
      };
    }
    ...
    for (const p of profiles) {
      ...
      const checkFilter = (userVal: any, filterArr: any[], fieldName: string) => {
        // Clean the filter array: remove empty strings and 'hepsi'
        const cleanFilter = (filterArr || []).filter(v => v && String(v).trim() !== '' && String(v).toLowerCase() !== 'hepsi');
        
        if (cleanFilter.length === 0)
          return 'match';
          
        if (!userVal) {
          // console.log(`[Matching] Field "${fieldName}" is missing for user`);
          return 'missing';
        }
        ...
        return cleanFilter
          ...
          .includes(lowerUserVal)
          ? 'match'
          : 'no_match';
      };

      const checks = [
        checkFilter(p.gender, survey.target_gender as any[], 'gender'),
        checkFilter(p.city, survey.target_city as any[], 'city'),
        checkFilter(p.education_level, survey.target_education as any[], 'education'),
        // Occupation logic: If not working, it's a match (doesn't block the user)
        p.work_status !== 'calisiyor'
          ? 'match'
          : checkFilter(p.occupation, survey.target_occupation as any[], 'occupation'),
        checkFilter(p.marital_status, survey.target_marital_status as any[], 'marital'),
        checkFilter(p.children_count, survey.target_child_count as any[], 'children'),
        checkFilter(p.work_status, survey.target_employment_status as any[], 'work_status'),
        checkFilter(p.household_income, survey.target_income as any[], 'income'),
        // Sector logic: If not working, it's a match
        p.work_status !== 'calisiyor'
          ? 'match'
          : checkFilter(p.sector_type, survey.target_sector as any[], 'sector'),
        // Position logic: If not working, it's a match
        p.work_status !== 'calisiyor'
          ? 'match'
          : checkFilter(p.position, survey.target_position as any[], 'position'),
        // IBAN Name check (required for profile completeness)
        !p.full_name_bank ? 'missing' : 'match',
        // Email check (required for notifications)
        !p.users?.email ? 'missing' : 'match',
      ];

      // Special check for Age
      if (
        survey.target_age_group?.length > 0 &&
        !survey.target_age_group.includes('hepsi')
      ) {
        if (!p.birth_date) {
          checks.push('missing');
        } else {
          ...
          if (
            ageGroup &&
            (survey.target_age_group as string[])
              .map((s: string) => s.toLowerCase())
              .includes(ageGroup.toLowerCase())
          ) {
            checks.push('match');
          } else {
      ...
      if (isMatch) {
        ...
        console.log(
          `[Matching] Incomplete data for: ${p.full_name} (${p.id}) - Checks: ${checks}`,
        );
        incomplete.push(userData);
      } else {
        console.log(
          `[Matching] No match for: ${p.full_name} (${p.id}) - Checks: ${checks}`,
        );
      }
    }

    console.log(
      `[Matching] Final results - Matches: ${matches.length}, Incomplete: ${incomplete.length}`,
    );
  ...
  async notifyMatchingUsers(surveyId: string, overrideUserIds?: string[]) {
    ...
    console.log(
      `[SurveysService] Starting notifications for survey: ${survey.title} (${surveyId})`,
    );
    ...
    if (overrideUserIds && overrideUserIds.length > 0) {
      ...
      console.log(
        `[SurveysService] No overrides, using getMatchingUsers logic.`,
      );
    ...
    for (const match of matches) {
      const rawEmail = match.users?.email || match.email;
      if (!rawEmail) {
        skipCount++;
        console.warn(
          `[SurveysService] Skipping user ${match.id} (${match.full_name}): No email found.`,
        );
        continue;
      }
      ...
      if (seenEmails.has(email)) {
        duplicateCount++;
        console.log(`[SurveysService] Skipping duplicate email: ${email}`);
        continue;
      }
      ...
      try {
        ...
        failCount++;
        console.error(
          `[SurveysService] Failed to notify user ${email}:`,
          err.message,
        );
      }
    }
    ...
    console.log(
      `[SurveysService] Notification Summary - Success: ${successCount}, Failed: ${failCount}, Skipped: ${skipCount}, Duplicates: ${duplicateCount}`,
    );
    ...
    if (
      supabaseUrl &&
      supabaseSecret &&
      supabaseSecret !== 'BURAYA_SECRET_GIRIN'
    ) {
      try {
        ...
        console.error(
          `[SurveysService] Failed to trigger Supabase function:`,
          err.message,
        );
      }
    }
