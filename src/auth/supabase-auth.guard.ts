    ...
    try {
      ...
      if (error || !data?.user) {
        throw new UnauthorizedException(
          `Invalid or expired token: ${error?.message || 'Unknown'}`,
        );
      }
      ...
      console.log(
        `Guard processing: User ID: ${data.user.id}, Email: ${data.user.email}, Role: ${profile.role}`,
      );
      ...
      request.user = {
        ...
        role: profile.role,
        is_researcher:
          (profile.role as any) === 'researcher' ||
